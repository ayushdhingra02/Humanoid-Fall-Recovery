...
if __name__=="__main__":
    ...
    obs,info=envs.reset()
    qpos_squat = [
        0, 0, 0.15,  # Lower body position (x, y, z)
    ...
    qvel_zero = np.zeros_like(envs.data.qvel)
    envs.data.qpos[:] = qpos_squat
    envs.data.qvel[:] = qvel_zero
    envs.render()
    while True:
        next_state, reward, done,_, _ = envs.step(np.zeros(22))
