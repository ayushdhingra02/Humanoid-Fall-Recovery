...
class HumanoidEnv(MujocoEnv):
    ...
    def __init__(
        ...
        self.observation_space = Box(
            low=-np.inf, high=np.inf, shape=(82,), dtype=np.float64
        )
    ...
    def step(self, action):
        ...
        return self.get_observation(), reward, self.terminated, False, {}
    ...
    def _compute_obs(self):
        ...
        obs = np.concatenate(
            [self.get_projected_gravity(), self.current_qpos - self.default_pos, calculated_qvel, self.action])
    ...
    def reward(self):
        ...
        for j in range(self.model.njnt):
            ...
            if joint_angle < -2.356 or joint_angle > 2.356:
        ...
        total_reward = (
                    height_reward + height_gain_reward + feet_contact_reward + feet_movement_penalty + oreintation_reward + done_reward + zmp_reward - joint_limit_penalty - angular_momentum_penalty - time_penalty - jerkiness_penalty)
        reward_info = {"height_reward": height_reward, "height_gain_reward": height_gain_reward,
                       "feet_contact_reward": feet_contact_reward, "feet_movement_penalty": feet_movement_penalty,
                       "oreintation_reward": oreintation_reward, "done_reward": done_reward, "zmp_reward": zmp_reward,
                       "joint_limit_penalty": joint_limit_penalty, "angular_momentum_penalty": angular_momentum_penalty,
                       "time_penalty": time_penalty, "jerkiness_penalty": jerkiness_penalty}
    ...
    def reset_model(self):
        ...
        gravity_vector = self.model.opt.gravity / np.linalg.norm(self.model.opt.gravity)
        observation = np.concatenate([gravity_vector, qpos_squat, qvel, np.zeros(22)])
