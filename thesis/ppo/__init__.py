...
from thesis.ppo.actor_crtic import ActorCritic
...
class Runner:
    ...
    def __init__(self, env,writer, device='cpu'):
        ...
        actor_critic = ActorCritic(self.env.num_obs,
                                   ...
                                   self.env.num_actions).to(self.device)
    ...
    def log_video(self, it):
        ...
        if len(frames) > 0:
            ...
            self.writer.add_video('Training Video', torch.tensor(frames).permute(0, 3, 1, 2), it, fps=1 / self.env.dt)
